...
if (!function_exists('sud_cast_component')) {
     function sud_cast_component($component, $col) {
          ...
          switch ($component['acf_fc_layout']) {
               case 'spacer':
                    $componentHTML .= '<div class="component-spacer" style="height: '.$component['height'].'px;" >';
                    $componentHTML .= '</div>';
                    break;
               case 'title':
               ...
               case 'button_group': 
                    ...
                    foreach ($component['buttons'] as $button) {
                         ...
                         switch($button['type']){
                              case 'link':
                                   $componentHTML .= '<a class="component-button" style="with:fit-content;" href="'.$button['buttonlink']['url'].'" target="'.$button['buttonlink']['target'].'">';
                                        $componentHTML .= '<div class="'.$button['button_type'].'" style="margin:0;">';  
                                             //$componentHTML .= $btnIcon;
                                             $componentHTML .= '<p style="margin:unset;">'.$button['buttontext'].'</p>';
                                        $componentHTML .= '</div>';
                                   $componentHTML .= '</a>';
                                   break;
                              case 'lb': 
                                   $targeting = 'lb-target-'.rand(100, 999);
                                   $componentHTML .= '<div class="component-button" style="with:fit-content; margin:0;" >';
                                        $componentHTML .= '<button class=" '.$button['button_type'].' fs" style="margin:0;" onclick="sudLightbox(\''.$targeting.'\')">'.$button['buttontext'].'</button>';
                                        //CONTENT
                                        $componentHTML .= '<div class="lightbox-wrapper" lbtarget="'.$targeting.'">'; 
                                             $componentHTML .= '<div class="lightbox-close-layer" lbcloser="'.$targeting.'" ><img src="'. get_template_directory_uri() .'/assets/img/utils/close-cross.svg" alt=""></div>';
                                             $componentHTML .= '<div class="lightbox-container" >'; 
...
<div style="display:<?php echo $hide; ?>;">
     <div <?php echo $anchor; ?>class="section-wrapper reveal" style="min-height:200px;">
          ...
          if($container_r && array_key_exists( "flex", $container_r ) ){
               ...
               $r_containerstyle = 'display:'.$r_flex.'; flex-wrap:'.$r_flexwrap.'; flex-direction:'.$r_direction.'; justify-content:'. $r_justifyContent.'; align-items:'.$r_alignItems .'; gap:'.$r_gap.'px; padding-top:'.$r_ptop.'; padding-right:'.$r_pright.'; padding-bottom:'.$r_pbottom.'; padding-left:'.$r_pleft.';';
          }
          ...
          if($container_l && array_key_exists( "flex", $container_l ) ) {
               ...
               $l_containerstyle = 'display:'.$l_flex.'; flex-wrap:'.$l_flexwrap.'; flex-direction:'.$l_direction.'; justify-content:'. $l_justifyContent.'; align-items:'.$l_alignItems .'; gap:'.$l_gap.'px; padding-top:'.$l_ptop.'; padding-right:'.$l_pright.'; padding-bottom:'.$l_pbottom.'; padding-left:'.$l_pleft.';';
          }
          ?>
          
               <div class="section-col section-col-l" style="width:<?php echo $colstyle['left-width'] . $l_containerstyle; ?>; ">
                    <?php
                    if($leftComponents) {
               ...
               <div class="section-col section-col-r" style="width:<?php echo $colstyle['right-width'] . $r_containerstyle; ?>; ">
                    <?php
                    if($rightComponents) {
