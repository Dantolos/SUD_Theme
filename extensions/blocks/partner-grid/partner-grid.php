...
if (!function_exists("render_partner_grid_standard")) {
    function render_partner_grid_standard(
        $title,
        $desc,
        $partners,
        $call_to_action
        ...
        foreach ($partners as $key => $partnerCat) {
            ...
            foreach ($partnerCat as $key => $partner) {
                ...
                $standard_grid .=
                    '<a href="' .
                    get_field("website", $partnerID) .
                    '" target="_blank">';
                $standard_grid .=
                    '<img src="' .
                    get_field("logo", $partnerID)["url"] .
                    '" alt="Logo ' .
                    get_field("company_name", $partnerID) .
                    '"/>';
                $standard_grid .= "</a>";
                $standard_grid .= "</div>";
            }
...
if (!function_exists("render_partner_grid_name_only")) {
    function render_partner_grid_name_only(
        string $title,
        string $desc,
        array $partners,
        $call_to_action
        ...
        foreach ($partners as $key => $partnerCat) {
            ...
            foreach ($partnerCat as $key => $partner) {
                ...
                $n_o_grid .= $partnerlink
                    ? '<a href="' .
                        get_field("website", $partnerID) .
                        '" target="_blank">'
                    : "";
                $n_o_grid .=
                    "<h6>" . get_field("company_name", $partnerID) . "</h6>";
...
<div class="<?php echo $hide_on_mobile; ?>" style="display:<?php echo $hide; ?>">
     <div <?php echo $anchor; ?>class="partner-grid-wrapper" >
          <?php switch ($style_type) {
              case "standard":
                  echo render_partner_grid_standard(
                      $section_title,
                      $section_desc,
                      $partners,
                      $call_to_action
                  ...
                  break;
              case "nameonly":
                  echo render_partner_grid_name_only(
                      $section_title,
                      $section_desc,
                      $partners,
                      $call_to_action
